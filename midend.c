...
/*
 * Apply an arbitrary move string as a normal, undoable move, exactly
 * as if the player had made it -- without going through
 * interpret_move()/a real UI event. Every game already knows how to
 * parse and apply its own move strings via execute_move(); this just
 * gives JS (via a per-game emscripten export, e.g. Keen's apply_move()
 * in emcc-ap.c) a way to hand one in directly, for features whose move
 * is computed client-side rather than generated by an actual click or
 * keypress (see Keen's "double-right-click a clued cage to pencil in
 * candidates" feature, which builds an "F..." move string in JS -- see
 * execute_move() in keen.c).
 *
 * Deliberately mirrors midend_restart_game()'s simpler push-a-new-
 * state pattern (no SOLVE_ANIMATES-style animation dance, since the
 * only mover of this function today never needs one and the ordinary
 * animate-and-redraw machinery in midend_really_process_key() isn't
 * reachable from outside a real key/mouse event), rather than
 * midend_solve()'s. movetype is plain MOVE, so undo/redo treat this
 * exactly like an ordinary player move -- one midend_undo() reverses
 * the whole thing, however many cells it touched.
 *
 * Returns NULL on success, or an error string (never needing to be
 * freed by the caller: either a literal, or NULL) if the move string
 * didn't parse/apply.
 */
const char *midend_apply_move(midend *me, const char *movestr)
{
    ...
    if (me->statepos < 1)
	return "No game set up";

    movestr_owned = dupstr(movestr);
    assert_printable_ascii(movestr_owned);
    s = me->ourgame->execute_move(me->states[me->statepos-1].state,
                                  movestr_owned);
    if (!s) {
	sfree(movestr_owned);
    ...
    me->states[me->nstates].state = s;
    me->states[me->nstates].movestr = movestr_owned;
    me->states[me->nstates].movetype = MOVE;
    me->statepos = ++me->nstates;
    if (me->ui)
        me->ourgame->changed_state(me->ui,
